...
module.exports = class myParty {
    ...
    async run(bot, message, args) {
        ...
        if (Player_info.Character_Party == "無") return message.reply("你沒有隊伍.").then(msg => {
            msg.delete(1000)
        });
        ...
        let partyEmbed = new Discord.RichEmbed()
            ...
            .addField("隊伍設定",
                "**目前狩獵目標：**`狩獵ID(" + Party_info.Party_Hunt_Target + ")`\n" +
                "**目前成員人數：**`" + Party_info.Party_Member_Number + "`\n" +
                "**分配設定：**`" + Party_info.Party_Share_Type + "`"
            )

        message.reply(partyEmbed).then(msg => {
            msg.delete(10000)
        });
    }
}