...
let checkItem = function (tradePlayer_info, itemID) {
    if (itemID == tradePlayer_info.invh_1.itemID) {
        tradePlayer_info.invh_1.itemID = "000";

    } else if (itemID == tradePlayer_info.inv_2.itemID) {
        tradePlayer_info.invh_2.itemID = "000";

    } else if (itemID == tradePlayer_info.inv_3.itemID) {
        tradePlayer_info.invh_3.itemID = "000";

    } 
}
module.exports = class trade_huntTarget {
    constructor() {
        this.name = 'trade_hunt',
            this.alias = ['交易狩獵目標', 'th'],
            this.usage = '!trade_hunt'
    }
    ...
    async run(bot, message, args) {
         ...
         */
        if (itemID != tradePlayer_info.invh_1.itemID &&
            itemID != tradePlayer_info.invh_2.itemID &&
            itemID != tradePlayer_info.invh_3.itemID ) {
             ...
             * 檢查得到道具玩家的背包是否已滿
             */
            if (getPlayer_info.invh_1.itemID == "000") {
                getPlayer_info.invh_1 = {
                    itemID: itemID
                }
                checkItem(tradePlayer_info, itemID);
            } else if (getPlayer_info.invh_2.itemID == "000") {
                getPlayer_info.invh_2 = {
                    itemID: itemID
                }
                checkItem(tradePlayer_info, itemID);
            } else if (getPlayer_info.invh_3.itemID == "000") {
                getPlayer_info.invh_3 = {
                    itemID: itemID
                }
                checkItem(tradePlayer_info, itemID);
            } else {
            ...
            message.reply("給予狩獵目標完成.").then(msg => {
                msg.delete(1000)
            });
