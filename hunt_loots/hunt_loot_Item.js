...
module.exports = {
    ...
    get_lootItem: function (playerID) {
        ...
        let choose_monster = player_Hunt[playerID].Monster_ID;
        if (choose_monster == "1000") { //魔物代號 1000
        ...
        } else if (choose_monster == "2000") { //魔物代號 1000
            lootItem_number = Math.floor(Math.random() * 2) + 1;
            console.log("取得裝備號碼:" + lootItem_number)
            switch (lootItem_number) {
                case 1:
    ...
    lootItem: function (playerID) {
        islootItem = Math.floor(Math.random() * 100) + 1;
        console.log("取得裝備是否:" + islootItem)

        if (islootItem <= 20 && islootItem > 0){
